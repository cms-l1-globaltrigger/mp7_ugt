...
def main():
    ...
    l1menu_html = menu_dir.split("/")[-1] + ".html"
    tme_version = config.get('tme', 'version')
    
    versions = {}
    versions['tm-vhdlproducer'] = detect_vhdl_producer_version(os.path.join(buildarea_dir, 'src', 'module_0', 'l1menu_pkg.vhd'))
    versions['tm-reporter'] = detect_tm_reporter_version(os.path.join(buildarea_dir, 'src', l1menu_html))
    versions.update(detect_gt_versions(os.path.join(buildarea_dir, 'src', 'mp7_ugt', 'firmware', 'hdl', 'packages', 'gt_mp7_core_pkg.vhd')))
